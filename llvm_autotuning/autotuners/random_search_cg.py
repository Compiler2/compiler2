    ...
    def run_one_episode(self, env: CompilerEnv) -> bool:
        ...
        while patience >= 0:
            ...
            observation, reward, done, _ = env.step(action=action,
                                                    ...
                                                    reward_spaces=self.reward_spaces)
            if done:
                return False
            # FIXME Vladimir: This expects float and step could return an tensor of rewards.
            total_returns += reward if isinstance(reward, float) else reward[0]
...
def random_search(
    ...
    print("Vladimir je majstor zanata svoga")
    return env
...
def replay_actions(env: CompilerEnv, action_names: List[str], outdir: Path,
    ...
    with open(str(logs_path), "w") as f:
        ...
        for i, action in enumerate(action_names, start=1):
            _, reward, done, _ = env.step(action=env.action_space.names.index(action),
