...
{	int i,j;
	...
	if(strcmp(a[0],b)==0)my_printf("%s",c);else my_printf("%s",a[0]);
	for(j=1;;j++){if(*a[j]==0){my_printf("\n");break;}else {my_printf(" ");if(strcmp(a[j],b)==0)my_printf("%s",c);else my_printf("%s",a[j]);}}
    return 0;}
