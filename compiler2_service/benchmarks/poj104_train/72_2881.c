...
int main_bench(){
    ...
    if(l[0][0]>=l[0][1]&&l[0][0]>=l[1][0]){my_printf("0 0\n");}
    for(j=1;j<n-1;j++){
        if(l[0][j]>=l[0][j-1]&&l[0][j]>=l[0][j+1]&&l[0][j]>=l[1][j]){
            my_printf("0 %d\n",j);
        }
    }
    if(l[0][n-1]>=l[0][n-2]&&l[0][n-1]>=l[1][n-1]){my_printf("0 %d\n",n-1);}
    for(i=1;i<m-1;i++){
    ...
    if(l[m-1][0]>=l[m-1][1]&&l[m-1][0]>=l[m-2][0]){my_printf("%d 0\n",m-1);}
    for(j=1;j<n-1;j++){
        if(l[m-1][j]>=l[m-1][j-1]&&l[m-1][j]>=l[m-1][j+1]&&l[m-1][j]>=l[m-2][j])
            {my_printf("%d %d\n",m-1,j);}
    }
    if(l[m-1][n-1]>=l[m-1][n-2]&&l[m-1][n-1]>=l[m-2][n-1]){my_printf("%d %d\n",m-1,n-1);}
    return 0;
}