...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, tgt_mask=None, src_pad_mask=None, tgt_pad_mask=None):
        ...
        memory = self.encoder(src, mask=src_pad_mask)
...
def train_loop(model, opt, loss_fn, dataloader):
    ...
    if lname == 'CrossEntropyLoss':
        loss = loss_fn(logits, dataloader['y'])
    elif lname == 'L1Loss':
        logits = logits[:, 0, :]
        nn.L1Loss(reduction="sum")(logits, dataloader['y'])
...
if __name__ == "__main__":
    ...
    train_loss_list, validation_loss_list = fit(model=model, opt=opt, loss_fn=loss_fn, train_dataloader=dgl_dataset.collate())
    ...
    logits = model(dgl_dataset.collate()['x'])


    # TODO: Figure out how to do this example by example
